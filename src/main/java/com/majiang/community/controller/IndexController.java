...
public class IndexController {
    ...
    public String index(
        ...
        List<String> hotTags = null;
        try {
            paginationDTO = questionService.list(page, size, search, tag);
            hotTags = hotTagCache.getHots();
        } catch (Exception e) {
            log.error("callback get github error,{}",paginationDTO);
            log.error("callback get github error,{}",hotTags);
        }
        model.addAttribute("paginationDTO", paginationDTO);
        //回显
        model.addAttribute("search", search);
        model.addAttribute("tag", tag);
        model.addAttribute("hotsTags", hotTags);
        return "index";
    }
}
